...
## 阶段二：电机 CAN 控制命令帧（SocketCAN）

完成时间：2026-07-22

- 使用 CAN ID `0x200` 表示 4 路电机控制命令帧；每路电机占用 2 字节，共 8 字节。
- 定义 `MotorCurrentCommand`，保存 4 路 `int16_t` 目标电流原始值。
- 使用大端字节序编码有符号 16 位数：
  - `1000` → `0x03 0xE8`
  - `-500` → `0xFE 0x0C`
  - `2500` → `0x09 0xC4`
- 将编码逻辑拆分为 `motor_command.hpp` 与 `motor_command.cpp`，使 SocketCAN 程序可以复用。
- `socketcan_command_demo.cpp` 在 `vcan0` 上发送并 loopback 接收 ID 为 `0x200` 的命令帧，验证 DLC 为 8、数据字节正确。
- 虚拟 CAN 不会驱动电机；真实 CAN 上的控制帧可能产生扭矩，必须以设备协议、限流、急停和上电安全流程为前提。
...
- 使用大端字节序编码有符号 16 位数：
  ...
  - `-500` → `0xFE 0x0C`
  - `2500` → `0x09 0xC4`
- 将编码逻辑拆分为 `motor_command.hpp` 与 `motor_command.cpp`。
- `socketcan_command_demo.cpp` 在 `vcan0` 上发送并 loopback 接收 ID 为 `0x200` 的命令帧，验证 DLC 为 8、数据字节正确。
- 虚拟 CAN 不会驱动电机；真实 CAN 控制帧可能产生扭矩，必须先确认设备协议、限流、急停和上电安全流程。

## 阶段二：CAN 电机命令安全边界

完成时间：2026-07-22

- 在 CAN 控制帧编码前增加安全层，避免异常控制值直接发送到电机驱动器。
- `make_safe_command()` 的规则：
  - 输出未使能时，4 路命令均清零；
  - 故障有效时，4 路命令均清零；
  - 正常状态下，将每路命令限幅到 `[-5000, 5000]`。
- 示例验证：请求 `{7000, -6000, 500, 0}` 在正常状态下变为 `{5000, -5000, 500, 0}`；故障状态下变为 `{0, 0, 0, 0}`。
- `5000` 只是练习用原始单位；真实项目必须依据驱动器协议、热保护和机械安全边界确定限值。

## 阶段二：电机 CAN 命令安全测试（CTest）
